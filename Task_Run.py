...
def parse_config():
    ...
    parser.add_argument(
        'FAMILY',
        metavar='[FAMILY]',
        help='Who owns the APP?'
    )
    parser.add_argument(
        'LANDSCAPE',
        metavar='[LANDSCAPE]',
        help='Location name of the APP?'
    )
    parser.add_argument(
        'ENVIRONMENT',
        metavar='[ENVIRONMENT]',
        help='Is this pre-prod or prod?'
    )
    parser.add_argument(
        'PRODUCT',
        metavar='[PRODUCT]',
    ...
    parser.add_argument(
        '--REGION',
        metavar='[REGION]',
        help='AWS region where this task will be run?'
    )
    parser.add_argument(
        '--IMAGE_VERSION',
        metavar='[IMAGE_VERSION]',
        help='Docker Image version? To be prefixed with "v"'
    )
    parser.add_argument(
        '--PARAM_VERSION',
        metavar='[PARAM_VERSION]',
        help='Verion of parameters? To be prefixed with "v"'
    )
    parser.add_argument(
        '--RAKE',
        metavar='[RAKE]',
        help='Full rake command encapsulated with double quotes'
    )
    ...
    config['image_url'] = (
        "700326549305.dkr.ecr.{}.amazonaws.com/{}/{}:{}".format(
            config['aws_region'],
            config['family'],
            config['product'].replace("-", "_"),
            args.IMAGE_VERSION
...
def build_task_definition_object(ecsClient, config, taskArn):
    ...
    railsProducts = ['stripe', 'central']
    task_object['containerDefinitions']['command'] = []

    for product in railsProducts:
        if product in config['product']:
            task_object['containerDefinitions']['command'] = [config['rake']]

    task_object['containerDefinitions']['image'] = config['image_url']
    (task_object['containerDefinitions']['logConfiguration']['options']
                ['awslogs-group']) = config['migration_log']
    print task_object
    return task_object
